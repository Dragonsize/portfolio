...
export function initLabAmbient() {
  ...
  const drawPulse = (pulse, time) => { const progress = Math.min(1, (time - pulse.startedAt) / pulse.duration); const stages = Math.max(...pulse.segments.map(({ depth }) => depth)) + 1; pulse.segments.forEach((segment) => { const local = Math.min(1, Math.max(0, progress * stages - segment.depth)); if (!local) return; const tail = Math.max(0, local - .18); drawTrail(segment.start, segment.end, tail, local, .9 * (1 - progress)); const x = segment.start.x + (segment.end.x - segment.start.x) * local; const y = segment.start.y + (segment.end.y - segment.start.y) * local; context.fillStyle = palette.glow; context.globalAlpha = .9 * (1 - progress); context.beginPath(); context.arc(x, y, 2.2, 0, Math.PI * 2); context.fill(); }); return progress < 1; };
