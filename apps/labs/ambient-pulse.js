export const PULSE_CONFIG = Object.freeze({ gridSize: 32, minCells: 4, maxCells: 10, minDelay: 1600, maxDelay: 4800, minDuration: 700, maxDuration: 1100, maxDeviceRatio: 1.25, maxCanvasPixels: 1250000 });
...
export function initLabAmbient() {
  ...
  let width = 0; let height = 0; let frame = 0; let timer = 0; let pulse = null; let palette = {};
  ...
  const clear = () => { cancelAnimationFrame(frame); clearTimeout(timer); frame = 0; timer = 0; pulse = null; context.clearRect(0, 0, width, height); };
  ...
  const animate = (time) => { if (!pulse || !canAnimate()) return clear(); const progress = Math.min(1, (time - pulse.startedAt) / pulse.duration); context.clearRect(0, 0, width, height); context.strokeStyle = palette.pulse; drawSegment(pulse.start, pulse.end, 0, 1, .12 * (1 - progress), .8); const tail = Math.max(0, progress - .18); drawSegment(pulse.start, pulse.end, tail, progress, .9 * (1 - progress * .35), 1.5); const x = pulse.start.x + (pulse.end.x - pulse.start.x) * progress; const y = pulse.start.y + (pulse.end.y - pulse.start.y) * progress; context.fillStyle = palette.glow; context.globalAlpha = .9 * (1 - progress * .3); context.beginPath(); context.arc(x, y, 2.2, 0, Math.PI * 2); context.fill(); context.globalAlpha = 1; if (progress < 1) { frame = requestAnimationFrame(animate); return; } frame = 0; pulse = null; context.clearRect(0, 0, width, height); schedule(); };
  const schedule = () => { if (!canAnimate() || timer || frame) return; const delay = PULSE_CONFIG.minDelay + Math.random() * (PULSE_CONFIG.maxDelay - PULSE_CONFIG.minDelay); timer = setTimeout(() => { timer = 0; const next = createPulse(width, height); if (!next) return; pulse = { ...next, startedAt: performance.now() }; frame = requestAnimationFrame(animate); }, delay); };
